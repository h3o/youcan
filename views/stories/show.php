...
function renderComment(array $c, bool $isReply, string $slug, bool $isStoryOwner): void
{
    ...
    <div class="<?= $cls ?>" id="comment-<?= (int)$c['id'] ?>">
        <div class="comment__header">
            ...
            <a href="<?= url('/profile/' . htmlspecialchars($c['username'], ENT_QUOTES, 'UTF-8')) ?>" class="comment__author">
                <?= htmlspecialchars($c['username'], ENT_QUOTES, 'UTF-8') ?>
            </a>
            <span class="comment__date"><?= date('M j, Y', strtotime($c['created_at'])) ?></span>
            <?php if ($isOwner): ?>
            <form method="post" action="<?= url('/comments/' . (int)$c['id'] . '/delete') ?>" class="comment__delete-form"
...
<div class="story-layout">
    ...
    <article class="story-reader">
        <header class="story-reader__header">
            ...
            <div class="story-reader__meta">
                <a href="<?= url('/profile/' . htmlspecialchars($story['username'], ENT_QUOTES, 'UTF-8')) ?>" class="story-reader__author">
        ...
        <footer class="story-reader__footer">
            ...
            <div class="story-reader__actions">
                <a href="<?= url('/stories/' . $slug . '/edit') ?>" class="btn btn--ghost btn--sm"><?= htmlspecialchars(t('story.edit'), ENT_QUOTES, 'UTF-8') ?></a>
                <form method="post" action="<?= url('/stories/' . $slug . '/delete') ?>" class="delete-form"
    ...
    <section class="comments-section" id="comments">
        ...
        <?php else: ?>
            <?php foreach ($storyComments as $c): ?>
                ...
                <?php if (Auth::check()): ?>
                <form method="post" action="<?= url('/stories/' . $slug . '/comments') ?>" class="comment-reply-form">
                    <?= Csrf::field() ?>
        ...
        <?php if (Auth::check()): ?>
        <form method="post" action="<?= url('/stories/' . $slug . '/comments') ?>" class="comment-form" id="new-story-comment">
            <?= Csrf::field() ?>
            <input type="hidden" name="type" value="story">
        ...
        <p class="comment-login-prompt">
            <a href="<?= url('/login') ?>"><?= htmlspecialchars(t('comment.login_prompt'), ENT_QUOTES, 'UTF-8') ?></a>
        </p>
        <?php endif; ?>
    </section>
  ...
  <aside class="annotation-panel" id="annotation-panel" aria-hidden="true" aria-label="Annotation panel">
    <div class="annotation-panel__inner">
        ...
        <?php if (Auth::check()): ?>
        <form class="annotation-panel__form" id="panel-form" method="post" action="<?= url('/stories/' . $slug . '/comments') ?>">
            <?= Csrf::field() ?>
